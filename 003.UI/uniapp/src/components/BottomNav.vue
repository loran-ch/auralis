<template>
  <view class="bottom-nav">
    <view class="nav-inner content-wide">
      <view v-for="item in items" :key="item.key" class="nav-item" :class="{ active: active === item.key }" @tap="navigate(item)">
...
const items = [
  ...
  { key: 'cards', label: '卡片', icon: '★', url: '/pages/cards/index' },
...
.nav-item { width: 25%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; color: var(--muted); }
